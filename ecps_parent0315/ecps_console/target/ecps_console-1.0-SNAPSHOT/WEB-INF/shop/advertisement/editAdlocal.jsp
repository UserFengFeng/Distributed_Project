	...
	*/
					...
					$.ajax({
			             ...
			             complete:function(data){
			            	...
			            	/*
			            		...
			            		alert('validsen.length002='+validsen.length);
			            	}
			            	*/
	...
	$("#sub").click(function(){
		var options = {
			...
			type: 'post',
			dataType: 'json',
			url: '${base}/adlocal/updateAdlocal.do'
		};
...
<div class="frameR"><div class="content">
	...
	<form id="fm" name="fm" action="" method="post" class="edit set">
		<p>
			<label for=""><samp>*</samp>广告位名称:</label><input type="text" id="locationName" value="${ebAdLocal.locationName}" name="locationName" class="txt"/><span class="gray">请输入4~16位的中英文或数字</span>
		</p>
        <p><label class="alg_t">广告位描述:</label><textarea id="locationDescription" name="locationDescription" rows="4" cols="60" value="${ebAdLocal.locationDescription}" >${ebAdLocal.locationDescription}</textarea><span class="gray">请输入140个字符内的广告位描述</span></p> 
		<p><label for=""><samp>*</samp>广告位尺度:</label>宽:<input type="text"  id="locationWidth" name="locationWidth" value="${ebAdLocal.locationWidth}"/>px&nbsp;&nbsp;&nbsp;高:<input type="text" id="locationHigh" name="locationHigh"  value="${ebAdLocal.locationHigh}"/>px<span class="gray">广告位宽、高:请输入1~950之间的整数</span></p>
		<p><label for=""></label><span class="red">修改广告位尺寸将会影响已投放广告的前台展示样式，请慎重考虑!”</span></p>
		
		<p><label for="submit">&nbsp;</label><input type="button" value="提 交" id="sub" class="hand btn83x23" />&nbsp;&nbsp;<input type="button" value="重 置" onclick="location='${base}/adlocal/editAdlocal.do?Id=${ebAdLocal.locationId}'"class="hand btn83x23b" /></p>
