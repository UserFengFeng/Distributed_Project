...
<div class="frameR"><div class="content">
    ...
    <div class="loc icon"><samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<a href="<c:url value="/${system }/simcard/listSimCard.do?saleStatus=1"/>" title="号卡管理">号卡管理</a>&nbsp;&raquo;&nbsp;<span class="gray">查看号卡</span>
    ...
    <form id="form111" name="form111" action="${path}/simcard/updateSimCard.do" method="post" enctype="multipart/form-data">
		<ul id="tab_1" class="uls edit set">
			...
			<li><label>号码标签：</label><span>&nbsp;</span><input type="button" id="editLabelsBtn" name="editLabelsBtn" value="编辑标签" class="pointer" disabled="disabled"/>
			</li>
			<li><label>&nbsp;</label>
				<div class="pre">
					<table id="tbodylbl" cellspacing="0" summary="" class="tab">
						...
						<tr id="lblIdTab${e.labelId}">
							...
							<td>${e.labelName}</td>
							<td><a href="javascript:void(0);" onclick="deleteLabel(${e.labelId});" ></a></td>
						</tr>
						</c:forEach>
					</table>
				</div>
			...
			<li><label>基本套餐：</label>
				<div class="pre">
					<table cellspacing="0" summary="" class="tab">
						...
						<c:forEach items="${suits}" var="e">
						<tr>
							<td>${e.suitNo}</td>
							<td class="nwp">${e.suitName}</td>
							<td>${e.suitFare}</td>
							<td>${e.brandName}</td>
							<td class="nwp"><c:out value="${e.suitDesc}" escapeXml="true"/></td>
						</tr>
						</c:forEach>
					</table>
				</div>
			...
			<li>
				<label>套卡价格：</label><fmt:formatNumber type="number" value="${card.simcardPrice/100}" pattern="#.##" minFractionDigits="2"></fmt:formatNumber>
				<input type="hidden" name="price" value="${card.simcardPrice/100 }"/>
				<%-- <input type="text" id="price" name="price" value="<fmt:formatNumber type="number" value="${card.simcardPrice/100}" pattern="#.##" minFractionDigits="2"></fmt:formatNumber>" class="text state" reg1="^(([1-9]+)|([0-9]+\.{0,1}[0-9]{0,2}))$" maxlength="10" desc="请输入大于0小于10000000的数字，最多保留两位小数！" errorspan="simCardPriceStyle"/><span id="simCardPriceStyle" class="pos"></span>--%>
			</li>
			<li>
				<label>预存金额：</label><fmt:formatNumber type="number" value="${card.prepaid/100}" pattern="#.##" minFractionDigits="2"></fmt:formatNumber>
			</li>
			<%--
			<li>
				<label>预存金额：</label><input type="text" id="price" name="price" value="<fmt:formatNumber type="number" value="${card.simcardPrice/100}" pattern="#.##" minFractionDigits="2"></fmt:formatNumber>" class="text state" reg1="^(([1-9]+)|([0-9]+\.{0,1}[0-9]{0,2}))$" maxlength="10" desc="请输入大于0小于10000000的数字，最多保留两位小数！" errorspan="simCardPriceStyle"/><span id="simCardPriceStyle" class="pos"></span>
			</li>
			 --%>
			<li><label>上下架状态：</label>
			<input type="radio" group="show" name="showStatus" value="1" class="txt" <c:if test="${card.showStatus==1}"> checked</c:if> disabled="disabled"/>上架&nbsp;&nbsp;
			<input type="radio" group="show" name="showStatus" value="0" class="txt" <c:if test="${card.showStatus==0}"> checked</c:if> disabled="disabled"/>下架
			</li>
		</ul>
		<%--
