...
<head>
<title>新增网银_<c:out value="${paymentOrg.poName}"></c:out>支持网银列表_支付机构管理_支付管理</title>
...
<body id="main">
	...
	<div class="frameR"><div class="content">
		<div class="loc icon"><samp class="t12"></samp><fmt:message key='menu.current.loc'/>：支付管理&nbsp;&raquo;&nbsp;<a href="<c:url value='/${system}/payment/listPaymentOrg.do'/>">支付机构管理</a>&nbsp;&raquo;&nbsp;<a href="<c:url value='/${system}/payment/listPayOlbank.do?poId=${paymentOrg.poId}'/>"><c:out value="${paymentOrg.poName}"></c:out>支持网银列表</a>&nbsp;&raquo;&nbsp;<span class="gray">新增网银</span>
	    <a href="<c:url value="/${system }/payment/listPayOlbank.do?poId=${paymentOrg.poId}"/>" title="返回实体商品" class="inb btn80x20">返回网银列表</a>
	    </div>
	    
	    
		<form id="form1" name="form1" action="${base}/payment/savePayOlbank.do" method="post">
			<div id="tab_1" class="edit set">
				<input type="hidden" id="poId" name="poId" value="<c:out value='${paymentOrg.poId}'></c:out>"/>
				<p><label>所属机构：</label><var><c:out value="${paymentOrg.poName}"></c:out></var>
				</p>
				<p><label><samp>*</samp>网银代码：</label><input type="text" id="olbankCode" name="olbankCode" class="text state" reg="^.+$" desc="网银代码不能为空！" err="网银代码不能为空！"/>
				<span></span>
				</p>
				<p><label><samp>*</samp>网银名称：</label><input type="text" id="olbankName" name="olbankName" class="text state" reg="^[a-zA-Z\u4e00-\u9fa5]{0,16}$" desc="请输入16位内的中英文" err="请输入16位内的中英文！"/>
				<span></span>
				</p>
				<p><label class="alg_t"><samp>*</samp>工具logo：</label><img id="olbankLogoImg" src="${path}/res/imgs/deflaut.jpg" height="42" width="151" />
				</p>
				<p><label></label><input type="file" id="olbankLogoFile" name="olbankLogoFile" class="file" onchange='submitUpload()' />
				<input type='hidden' id='olbankLogo' name='olbankLogo'/>
				...
				<p id="olbankDescP" style="display: none"><label>说明文字：</label><input type="text" id="olbankDesc" name="olbankDesc" class="text state" reg="^.{0,12}$" desc="请输入12位内的中英文、数字和符号" err="请输入12位内的中英文、数字和符号！"/>
				<span></span>
				</p>
				<p id="olbankDescUrlP" style="display: none"><label>链接地址：</label><input type="text" id="olbankDescUrl" name="olbankDescUrl" class="text state" reg="^(http:\/\/).{0,245}$" desc="请以http://开头，250个字符以内" err="请以http://开头，250个字符以内！"/>
				<span></span>
				</p>
