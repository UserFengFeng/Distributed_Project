...
<script type="text/javascript">
	...
	function showMemo(memo){
		tipShow("#showMemoDiv");
		$("#showMemoText").val(memo);
	}
	
	function exportRefundRecord(){
		var form = document.getElementById("form1");
		form.action="${base}/payment/exportRefundRecord.do";
		form.submit();
		form.action="${base}/payment/listRefundRecord.do";
	}
...
<body id="main">
	...
	<div class="frameR"><div class="content" style="overflow:auto" >
		<div class="loc icon"><samp class="t12"></samp><fmt:message key='menu.current.loc'/>：支付管理&nbsp;&raquo;&nbsp;<span class="gray">退款单管理</span>
	    </div>
	    
		<form id="form1" name="form1" action="${base}/payment/listRefundRecord.do" method="post">
			<div class="sch">
				<p>
					订单号：
					<c:choose>
					...
					<span id="searchTip2" class="red" style="display:none">结束日期不能小于起始日期!</span>
				</p>
			</div>
	    	<div class="page_c">
		        <span class="l">
		        ...
		        <span class="r inb_a">
		            <input type="button"  class="btn80x20" value="导出查询结果" onclick="exportRefundRecord()" />
		        </span>
	  		</div>
				...
				<tbody>
					<c:forEach items="${pagination.list}" var="payRecord">
						<tr>
							...
							<td><fmt:formatDate value="${payRecord.tradeRespdate}"pattern="yyyy-MM-dd" /></td>
							...
							<td><fmt:formatNumber value="${payRecord.orderSum/100}" pattern="#0.00" /></td>
							<td><c:out value="RMB"></c:out></td>
							<td><fmt:formatNumber value="${payRecord.tradeAmount/100}" pattern="#0.00" /></td>
							<td><c:out value="辽宁移动"></c:out></td>
							<td>
								<c:choose>
									<c:when test="${payRecord.tradeStatus==1}">退款创建</c:when>
									<c:when test="${payRecord.tradeStatus==2}">退款成功</c:when>
									<c:when test="${payRecord.tradeStatus==3}">退款关闭</c:when>
									<c:when test="${payRecord.tradeStatus==4}">退款撤销</c:when>
									<c:when test="${payRecord.tradeStatus==5}">退款失败</c:when>
							...
							<td><c:if test="${payRecord.memo!=null}"><a href="###" onclick="showMemo('<c:out value='${payRecord.memo}'></c:out>')">查看</a></c:if></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
