...
<input type="hidden" id="sensitiveListUrl" name="sensitiveListUrl" value="${base}/contentset/sensitive/listSensitive.do" />
    <div class="loc icon"><samp class="t12"></samp><fmt:message key='menu.current.loc'/>：系统配置&nbsp;&raquo;&nbsp;<a href="<c:url value="/${system}/contentset/sensitive/listSensitive.do"/>" title="敏感词管理">敏感词管理</a>&nbsp;&raquo;&nbsp;<span class="gray" title="添加敏感词">添加敏感词</span><a href="<c:url value="/${system}/contentset/sensitive/listSensitive.do"/>" title="返回敏感词管理" class="inb btn120x20">返回敏感词管理</a></div>
	<form id="form1" name="form1" action="${base}/contentset/sensitive/addSensitive.do" method="post">
		<div class="edit set">
            <p>
            	<label class="alg_t">敏感词名称：</label>
            	<textarea rows="4" cols="21" name="sensitiveName" class="are" reg="^[a-zA-Z0-9\u4e00-\u9fa5]{1,20}$" tip="必须是中英文或数字字符，长度1-20"></textarea>
            	<span><c:out value="${sensitiveName}"/></span>
            </p>
            <p>
            	<label>状态：</label><input type="radio" name="status" value="1" checked/>启用&nbsp;&nbsp;
            	<input type="radio" name="status" value="0" />停用
            </p>
            <p>
            	<label>&nbsp;</label>
            	<input type="submit" class="hand btn83x23" value="<fmt:message key='tag.confirm'/>" />
            	<input type="button" class="hand btn83x23b" value="取消" onclick="backToBackList()" />
            </p>
        </div>
	</form>
    <div class="loc">&nbsp;</div>
