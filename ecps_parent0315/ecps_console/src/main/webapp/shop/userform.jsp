...
<div class="frameL"><div class="menu icon">
    <h2 title="<fmt:message key="mainMenu.commonMenu"/>"><samp class="t03"></samp><fmt:message key="mainMenu.commonMenu"/></h2>
    <ul class="ul">
        <li class="here"><a href="<c:url value='/userform'/>"><samp class="t09"></samp><fmt:message key="menu.user"/></a></li>
        <li><a href="<c:url value='/fileupload'/>"><samp class="t10"></samp><fmt:message key="menu.selectFile"/></a></li>
    </ul>
</div></div>
...
<div class="loc icon"><samp class="t12"></samp><fmt:message key="userProfile.heading"/></div>
...
<form:form commandName="user" method="post" action="userform" onsubmit="return onFormSubmit(this)" id="userForm" class="sign">
<form:hidden path="id"/>
...
<ul class="ul set">
    <li>
        ...
        <input type="submit" class="hand btn83x23" name="save" onclick="bCancel=false" value="<fmt:message key="button.save"/>"/>
        <c:if test="${param.from == 'list' and param.method != 'Add'}">
            <input type="submit" class="hand btn83x23b" name="delete" onclick="bCancel=true;return confirmDelete('user')" value="<fmt:message key="button.delete"/>"/>
        </c:if>
        <input type="submit" class="hand btn83x23b" name="cancel" onclick="bCancel=true" value='<fmt:message key="button.cancel"/>'/>
    </li>
    <li>
        <appfuse:label styleClass="desc" key="user.username"/>
        <form:errors path="username" cssClass="fieldError"/>
        <form:input path="username" id="username" cssClass="text state" cssErrorClass="text state error"/>
    </li>
    <c:if test="${cookieLogin != 'true'}">
    <li>
        <appfuse:label styleClass="desc" key="user.password"/>
        <form:errors path="password" cssClass="fieldError"/>
        <form:password path="password" id="password" cssClass="text state" cssErrorClass="text state error" onchange="passwordChanged(this)" showPassword="true"/>
    </li>
    <li>
        <appfuse:label styleClass="desc" key="user.confirmPassword"/>
        <form:errors path="confirmPassword" cssClass="fieldError"/>
        <form:password path="confirmPassword" id="confirmPassword" cssClass="text state" cssErrorClass="text state error" showPassword="true"/>
    </li>
    </c:if>
    <li>
        <appfuse:label styleClass="desc" key="user.passwordHint"/>
        <form:errors path="passwordHint" cssClass="fieldError"/>
        <form:input path="passwordHint" id="passwordHint" cssClass="text state" cssErrorClass="text state error"/>
    </li>
    <li>
        <appfuse:label styleClass="desc" key="user.firstName"/>
        <form:errors path="firstName" cssClass="fieldError"/>
        <form:input path="firstName" id="firstName" cssClass="text state" cssErrorClass="text state error" maxlength="50"/>
    </li>
    <li>
        <appfuse:label styleClass="desc" key="user.lastName"/>
        <form:errors path="lastName" cssClass="fieldError"/>
        <form:input path="lastName" id="lastName" cssClass="text state" cssErrorClass="text state error" maxlength="50"/>
    </li>
    <li>
        <appfuse:label styleClass="desc" key="user.email"/>
        <form:errors path="email" cssClass="fieldError"/>
        <form:input path="email" id="email" cssClass="text state" cssErrorClass="text state error"/>
    </li>
    <li>
        <appfuse:label styleClass="desc" key="user.phoneNumber"/>
        <form:errors path="phoneNumber" cssClass="fieldError"/>
        <form:input path="phoneNumber" id="phoneNumber" cssClass="text state" cssErrorClass="text state error"/>
    </li>
    <li>
        <appfuse:label styleClass="desc" key="user.website"/>
        <form:errors path="website" cssClass="fieldError"/>
        <form:input path="website" id="website" cssClass="text state" cssErrorClass="text state error"/>
    </li>
    <li>
        ...
        <ul class="ul group">
            ...
            <li>
                ...
                <form:input path="address.city" id="address.city" cssClass="text large" cssErrorClass="text large error"/>
                <form:errors path="address.city" cssClass="fieldError"/>
            </li>
            <li>
                <appfuse:label key="user.address.province"/>
                <form:input path="address.province" id="address.province" cssClass="text large" cssErrorClass="text large error"/>
                <form:errors path="address.province" cssClass="fieldError"/>
            </li>
            <li>
                <appfuse:label key="user.address.postalCode"/>
                <form:input path="address.postalCode" id="address.postalCode" cssClass="text large" cssErrorClass="text large error"/>
...
<c:choose>
 ...
 <li><label class="desc">&nbsp;</label>
  <input type="submit" class="hand btn83x23" name="save" onclick="bCancel=false" value="<fmt:message key="button.save"/>" />
  <c:if test="${param.from == 'list' and param.method != 'Add'}">
      <input type="submit" class="hand btn83x23b" name="delete" onclick="bCancel=true;return confirmDelete('user')" value="<fmt:message key="button.delete"/>" />
  </c:if>
  <input type="reset" class="hand btn83x23b" name="cancel" onclick="bCancel=true" value='<fmt:message key="button.cancel"/>' />
</li>
</ul>
