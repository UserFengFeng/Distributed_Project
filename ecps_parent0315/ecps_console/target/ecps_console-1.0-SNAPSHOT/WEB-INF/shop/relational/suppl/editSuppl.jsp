...
<head>
<title>编辑供应商_<fmt:message key="SupplierMgmtMenu.title" />_<fmt:message key="RelationShipMgmtMenu.title"/></title>
<meta name="heading" content="编辑供应商"/>
...
function checkNameFormat(name) {
	if (!(/^[a-zA-Z\u4e00-\u9fa50-9()]{1,200}$/.test(name))) {
		return false;
	} else {
		return true;
	}
...
function submitUpload(componentName) {
	...
	$('#supplUpdate').ajaxSubmit(options);
}
...
<div class="frameR"><div class="content">
    ...
    <div class="loc icon">
        ...
        <samp class="t12"></samp><fmt:message key='menu.current.loc'/>：合作伙伴管理&nbsp;&raquo;&nbsp;<a href="${list}" title="供应商管理">供应商管理</a>&nbsp;&raquo;&nbsp;<span class="gray" title="编辑供应商">编辑供应商</span>
        <a href="${list}" title="返回供应商管理" class="inb btn120x20">返回供应商管理</a>
    </div>

    <form id="supplUpdate" name="supplUpdate" action="${base}/suppl/updateSuppl.do" method="post">
			
	<div class="edit set">
		<input type="hidden" id="supplId" name="supplId" value="${item.supplId}" />
	
		<h2 title="注册信息">注册信息</h2>

		<p>
			<label><samp>*</samp>供应商名称：</label><input type="text" id="supplName" name="supplName" class="text state" maxlength="20" value="${item.supplName}" reg1="^[a-zA-Z\u4e00-\u9fa51-9()]{1,20}$" desc="只允许输入1-20个英文汉字数字字符或括号"/>
			<span id="spanSupplName"></span>
		</p>
		
		<p>
			<label>供应商曾用名：</label><input type="text" id="fka" name="fka" class="text state" maxlength="20" value="${item.fka}" reg1="^[a-zA-Z\u4e00-\u9fa51-9()]{0,20}$" desc="只允许输入0-20个英文汉字数字字符或括号"/>
			<span id="spanFka"></span>
		</p>
		
		<p>
			<label>法人代表：</label><input type="text" id="legalName" name="legalName" class="text state" maxlength="20" value="${item.legalName}" reg1="^[a-zA-Z\u4e00-\u9fa5\s]{0,20}$" desc="只允许输入0-20个英文汉字数字字符或空格" />
			<span id="spanLegalName"></span>
		</p>
		
		<p>
			<label>注册资本：</label><input type="text" id="regCapital" name="regCapital" class="text state" maxlength="20" value="${item.regCapital}" reg1="^[0-9\u4e00-\u9fa5]{0,20}$" desc="只允许输入0-20个数字或汉字" />
		...
		<p>
			<label>营业执照号：</label><input type="text" id="licenseNo" name="licenseNo"  class="text state" maxlength="30" value="${item.licenseNo}" reg1="^[\d\-]{0,30}$" desc='只允许输入0-30个数字字符或"-"' />
	...
	<div class="edit set">
		...
		<p>
			<label><samp>*</samp>联系人：</label><input type="text" id="contact" name="contact" class="text state" maxlength="20" value="${item.contact}" reg1="^[a-zA-Z\u4e00-\u9fa5\s]{1,20}$" desc="只允许输入1-20个英文中文字符或空格" />
			<span id="spanContact"></span>
		</p>
		
		<p>
			<label>移动电话：</label><input type="text" id="mobile" name="mobile" class="text state" value="${item.mobile}" reg1="^(|1[3|4|5|8]{1}[0-9]{9})$" desc="移动电话为11位数字，并且需要输入正确的格式！" />
			<span id="spanMobile"></span>
		</p>
		
		<p>
			<label><samp>*</samp>固定电话：</label><input type="text" id="phone" name="phone" class="text state" maxlength="15" value="${item.phone}" reg1="^[\d\-]{1,15}$" desc='只允许输入1-15个数字字符或"-"' />
			<span id="spanPhone"><span class="gray">格式：xxx-xxxxxxxx</span></span>
		</p>
		
		<p>
			<label>传真：</label><input type="text" id="fax" name="fax" class="text state" value="${item.fax}" maxlength="15" reg1="^[\d\-]{0,15}$" desc='只允许输入1-15个数字字符或"-"' />
			<span id="spanPhone"><span class="gray">格式：xxx-xxxxxxxx</span></span>
		</p>
		
		<p>
			<label>Email：</label><input type="text" id="email" name="email"  class="text state" value="${item.email}" maxlength="50" reg1="^(|(\w)+(\.\w+)*@(\w)+((\.\w+)+))$" desc="email不能大于50个字符，并且需要输入正确的格式！" />
			<span id="spanEmail"></span>
		</p>
		
		<p>
			<label>网址：</label><input type="text" id="webSite" name="webSite"  class="text state" value="${item.webSite}" maxlength="50" reg1="^(|[^\u4e00-\u9fa5]{1,50})$" desc="只允许输入1-50个非汉字字符" />
			<span id="spanWebSite"></span>
		</p>
		
		<p>
			<label><samp>*</samp>地址：</label><input type="text" id="address" name="address"  class="text state" value="${item.address}" maxlength="50" reg1="^[a-zA-Z\u4e00-\u9fa5\s0-9]{1,50}$" desc="只允许输入0-50个英文汉字数字字符或空格" />
			<span id="spanAddress"></span>
		</p>
		
		<p>
			<label>邮编：</label><input type="text" id="zip" name="zip"  class="text state" value="${item.zip}" reg1="^(|\d{6})$" desc="邮政编码不能为空或者只能为6位数字！" />
	...
	<div class="edit set">
		...
		<p>
			<label><samp>*</samp>开户行：</label><input type="text" id="bankName" name="bankName" class="text state" value="${item.bankName}" maxlength="50" reg1="^[a-zA-Z\u4e00-\u9fa5\s0-9]{1,50}$" desc="只允许输入1-50个英文汉字数字字符或者空格" />
			<span id="spanBankName"></span>
		</p>
		
		<p>
			<label><samp>*</samp>开户行行号：</label><input type="text" id="bankNo" name="bankNo" class="text state" value="${item.bankNo}" reg1="^[\d]{12}$" desc="开户行号必填，必须为12位数字，并且需要输入正确的格式！" />
			<span id="spanBankNo"></span>
		</p>
		
		<p>
			<label><samp>*</samp>银行账号：</label><input type="text" id="bankAccount" name="bankAccount" class="text state" value="${item.bankAccount}" maxlength="30" reg1="^[\d]{1,30}$" desc="只允许输入1-30个数字" />
			<span id="spanBankAccount"></span>
		</p>
		
		<p>
			<label>纳税号：</label><input type="text" id="taxNo" name="taxNo" class="text state" value="${item.taxNo}" reg1="^(|[0-9XLxl]{15})$" desc="纳税号为15个字符，并且需要输入正确的格式！" />
	...
	<div class="edit set">
		...
		<p>
			<label><samp>*</samp>供货类型：</label><select id="supplType" name="supplType">
				<option value="1" <c:if test='${item.supplType==1}'>selected</c:if> >手机商家</option>
				<option value="2" <c:if test='${item.supplType==2}'>selected</c:if> >号卡商家</option>
			</select>
	...
	<div class="edit set">
		<p>
			<label for="button">&nbsp;</label><input type="button" value="确定" onclick="updateSuppl()" class="hand btn102x26" /><input type="button" value="取消" onclick="backToBackList()" class="hand btn102x26b" />
		</p>
	</div>
