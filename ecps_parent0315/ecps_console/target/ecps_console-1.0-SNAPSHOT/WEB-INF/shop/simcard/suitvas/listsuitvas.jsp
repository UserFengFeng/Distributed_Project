...
<div class="loc icon"><samp class="t12"></samp><fmt:message key='menu.current.loc'/>：号卡管理&nbsp;&raquo;&nbsp;<span class="gray">增值策划管理</span></div>

<form id="form1" name="form1" action="${base}/simcard/listSuitVas.do" method="post">
    <div class="sch">
        <p>
	        ...
	        <input type="text" id="suitVasName" name="suitVasName" value="<c:out value='${suitVasName}' escapeXml='true'/>" title="请输入增值策划名称" class="text20 medium gray" />
	...
	<table cellspacing="0" summary="" class="tab">
		...
		<tbody>
			<c:forEach items='${pagination.list}' var="suitVas">
			<tr>
				<td><c:out value='${suitVas.suitVasId}'/></td>
				<td><c:out value='${suitVas.suitVasName}' escapeXml='true'/></td>
				<td>
				...
				<td>
					<c:forEach items='${suitVas.suitNames }' begin='0' end='4' var='suitName' varStatus='p'>
						<c:if test='${p.count > 1 }'>|</c:if>
						<c:out value='${suitName }' escapeXml='true'/>
					</c:forEach>
					<c:if test="${fn:length(suitVas.suitNames)>5}">
						等<c:out value="fn:length(suitVas.suitNames)"></c:out>个
					</c:if>
				</td>
				<td><c:out value='${suitVas.suitVasSort }'/></td>
				<td>
					<a href="${base}/simcard/editSuitVas.do?suitVasId=${suitVas.suitVasId}&suitId=${suitId}">编辑</a>
				</td>
			</tr>
			</c:forEach>
		</tbody>
