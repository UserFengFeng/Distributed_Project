...
$(document).ready(function(){
	...
	searchText('#giftInfo','#labelName',50);
	//查询商品信息
	...
	**/
	...
	$("input").each(function(){
		if($(this).attr("name") == "secKillCount"){
			...
			if(!reg_secKillCount.test(temp_secKillCount) || $(this).val()==0){
				alert("适用商品的秒杀限量必须为1-10位的整数");
	...
	if(secKillOrder != ''){
		...
		if(!reg_secKillOrder.test(secKillOrder) || parseInt(secKillOrder)<=0){
			$("#secKillOrderErr").html("必须是0-3位正整数");
			return false;
		}else{
...
function  beforeSubmitForm(){
	//如果限购和限量为空则给其赋值为0
	var limitTimes = $("#limitSecKillTimes").val();
	if(limitTimes=="" || limitTimes=='undefined' || limitTimes==null){
		$("#limitSecKillTimes").val(0);
	}
	var limitCount = $("#limitSecKillCount").val();
	if(limitCount=="" || limitCount==null){
...
function viewTable(pagination){
   ...
   for(var i=0;i<len;i++){
       ...
       for(var ii=0;ii<rows;ii++){
           ...
           var secKillCount =pagination.list[i].listsku[ii].secKillCount;
           ...
           if(specValues != '' || true){
               ...
               content+="<td class='nwp'>";
                 ...
                 //-------xuchen
                   ...
                   '","specValues":"'+specValues+'","skuPrice":"'+skuPrice+'","secKillPrice":"'+secKillPrice+'","secKillCount":"'+secKillCount+'","itemStatus":"'+itemStatus+'","skuStatus":"'+skuStatus+'"}'
...
<div class="frameR"><div class="content">
	...
	<div class="loc icon"><samp class="t12"></samp><fmt:message key='menu.current.loc'/>：促销活动&nbsp;&raquo;&nbsp;<span class="gray">添加秒杀</span>
    </div>
	<form id="form111" name="form111" action="${base}/secKill/addSecKill.do" method="post" enctype="multipart/form-data">
		<div class="edit set">
			<p><label><samp>*</samp>活动类型：</label>
				<select id="activityType" name="activityType">
					<option value="0">秒杀活动</option>
				</select>
			</p>
...
<!-- 		                <th>&nbsp;</th> -->
			...
			<p><label><samp>*</samp>秒杀标题：</label><input type="text" id="secKillTitle" name="secKillTitle" maxlength="180" size="180" class="text state" reg="^(.{1,180})$" tip="180以内任意字符"/>
				<span class="pos" id="secKillTitleErr"></span>
			</p>
            <p><label class="alg_t">秒杀图片：</label><img id="secKillImgImgSrc" src="<c:url value='/ecps/console/images/logo266x64.png'/>" height="100" width="100" /></p>
            ...
            <input type='hidden' id='secKillImg' name='secKillImg' />
            <input type="hidden" id="isSecKillImgSuccess" value=""/>
            </p>
			<p><label>赠品信息：</label><input type="text" id="giftInfo" name="giftInfo" class="text state" maxlength="50"  size="" reg1="^(.{1,50})$" desc="50以内任意字符"/>
				<span class="pos">&nbsp;</span>
			</p>
			...
			<p><label>排序：</label><input type="text" name="secKillOrder" id="secKillOrder" class="text state" maxlength="3"  reg="^[0-9]{0,3}$" tip="必须是0-3位正整数"/>
				<span class="pos" id="secKillOrderErr">&nbsp;</span>
			</p>
			<p><label><samp>*</samp>有效时间：</label><input type="text" name="startTimeStr" id="startTime" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" class="text20 state date" reg="^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$" />&nbsp;-&nbsp;<input type="text" name="endTimeStr" id="endTime" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" class="text20 state date" reg="^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$" />
			...
			<p><label>限制用户秒杀次数：</label>
				...
				<input type="text" name="limitSecKillTimes" id="limitSecKillTimes" value="0" style="display:none"  reg="^[0-9]{0,11}$" tip="请输入大于0的整数" maxlength="10"/><span class="pos" id="limitTimesSpan"></span>
				<!-- <span class="pos">&nbsp;</span> -->
			</p>
			<p><label>限制单笔秒杀数量：</label>
				...
				<input type="text" name="limitSecKillCount" id="limitSecKillCount" value="0" style="display:none" reg="^[0-9]{0,11}$" tip="请输入大于0的整数" maxlength="10"/><span class="pos" id="limitCountSpan"></span>
				<!-- <span class="pos">&nbsp;</span> -->
			</p>
			<div class="wp92"><label class="alg_t">温馨提示：</label><div class="pre"><textarea name="info" id="info" ></textarea></div>
				<script type="text/javascript">   
			...
			<div class="loc">&nbsp;</div>
			<div class="edit set">
				<input type="hidden" id="noticeOrder" name="noticeOrder" value="0"/>
